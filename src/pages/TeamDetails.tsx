...
export const TeamDetails = () => {
  ...
  if (!team) {
    return <div className="p-8 text-center text-white">Time não encontrado.</div>;
  }

  return (
    <div className="p-4 md:p-0 pt-8 md:pt-0 space-y-6 pb-20">
      <Button variant="ghost" onClick={() => navigate(-1)} className="mb-2 pl-0 hover:bg-transparent text-gray-400 hover:text-white">
      ...
      <div className="flex border-b border-muted overflow-x-auto">
        {['overview', 'squad', 'history'].map((tab) => (
          <button
            ...
            className={`px-6 py-3 text-sm font-medium capitalize whitespace-nowrap transition-colors relative ${activeTab === tab ? 'text-primary' : 'text-gray-400 hover:text-white'
              }`}
          >
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              <section>
                <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">Próxima Partida</h3>
                <Card className="flex justify-between items-center">
                  ...
                  <div className="text-center px-4">
                    <div className="text-xs text-gray-500 mb-1">Brasileirão</div>
                    <div className="font-mono text-xl text-white font-bold">16:00</div>
                    <Badge variant="warning">DOM</Badge>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-white">Rival</span>
              ...
              <section>
                <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">Forma Recente</h3>
                <div className="flex gap-2">
                  {['W', 'W', 'D', 'L', 'W'].map((res, i) => (
                    <div key={i} className={`w-8 h-8 rounded flex items-center justify-center text-xs font-bold ${res === 'W' ? 'bg-green-500/20 text-green-500 border border-green-500/30' :
                      res === 'D' ? 'bg-gray-500/20 text-gray-400 border border-gray-500/30' :
              ...
              <Card className="bg-surface/50">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">Gols Marcados</span>
                    <span className="text-white font-mono">45</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">Posse Média</span>
                    <span className="text-white font-mono">58%</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">Cartões Amarelos</span>
          ...
          <div className="space-y-8">
            ...
            <div className="relative border-l-2 border-muted ml-3 space-y-8 pb-8">
              <div className="relative pl-8">
                ...
                <h4 className="text-white font-bold">Busca pelo Tetra</h4>
                <p className="text-sm text-gray-400 mt-1">O clube investe pesado em contratações para conquistar a América novamente.</p>
              </div>
              <div className="relative pl-8 opacity-70">
                <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-surface border-2 border-gray-500"></div>
                <span className="text-xs font-mono text-gray-500 mb-1 block">2019</span>
                <h4 className="text-white font-bold">O Ano Mágico</h4>
                <p className="text-sm text-gray-400 mt-1">Campeão da Libertadores e do Brasileiro no mesmo fim de semana.</p>
              </div>
              <div className="relative pl-8 opacity-50">
                <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-surface border-2 border-gray-500"></div>
                <span className="text-xs font-mono text-gray-500 mb-1 block">1981</span>
                <h4 className="text-white font-bold">Campeão do Mundo</h4>
                <p className="text-sm text-gray-400 mt-1">A era de ouro liderada por Zico conquista o planeta no Japão.</p>
              </div>
            </div>
          </div>
        )}
